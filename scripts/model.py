...
class OpenAIModel(BaseModel):
    ...
    def get_model_response(self, prompt: str, images: List[str]) -> (bool, str):
        ...
        response = requests.post(
            self.base_url, headers=headers, json=payload, proxies=proxies).json()
...
class QaModel(BaseModel):
    def __init__(self,ak):
        super().__init__()
        self.base_url = "https://fmh.alibaba-inc.com/api/azure/chat/vision/list"
        self.ak = ak
    @retry(stop_max_attempt_number=5, wait_exponential_multiplier=1000)
    def get_model_response(self, prompt: str, images: List[str]) -> (bool, str):
        print_with_color(f"QaModel.get_model_response:\n prompt:\n{prompt}\n images:\n{images}\n", "blue")
        base64_imgs = [encode_image(image) for image in images]
        headers = {
            "Content-Type": "application/json",
            "Authorization": self.ak
        }
        body ={
            "prompt": prompt,
            "images": base64_imgs
        ...
        response = requests.post(
            self.base_url, headers=headers, json=body).json()
        print_with_color(f"QaModel response:\n {response}")
        # if not response.get("data"):
        #     return False, response
        if not response.get("data") or not response.get("success"):
            print_with_color(f"QaModel.get_model_response Error res:\n{response}")
...
def parse_explore_rsp(rsp):
    try:
        ...
        print_with_color(
            f"ERROR: parse_explore_rsp an exception occurs while parsing the model response: {e}", "red")
...
def parse_grid_rsp(rsp):
    try:
        ...
        print_with_color(
            f"ERROR: parse_grid_rsp an exception occurs while parsing the model response: {e}", "red")
        print_with_color(rsp, "red")
        return ["ERROR"]
...
def parse_reflect_rsp(rsp):
    try:
        ...
        print_with_color(
            f"ERROR: parse_reflect_rsp an exception occurs while parsing the model response: {e}", "red")
        print_with_color(rsp, "red")
        return ["ERROR"]
